      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          <div className="text-center">
            ...
            <h2 className="text-h2 font-serif text-brand mb-2">
              {reportData.personalLabel.label}
            </h2>
            <p className="text-body-lg text-text-secondary">{reportData.personalLabel.description}</p>
          </div>
        </motion.section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {reportData.top5Strengths.map((strength) => (
              <motion.div
                ...
                className="bg-bg-secondary rounded-xl p-4 border border-border-light shadow-inner-soft"
              >
                <div className="flex items-center gap-3 mb-2">
                  <div className="w-8 h-8 rounded-full bg-brand/10 flex items-center justify-center shadow-soft">
                    <span className="text-sm font-bold text-brand">{strength.rank}</span>
                  </div>
                  <div>
                    <p className="font-medium text-text-primary text-body-sm">{strength.name}</p>
                    <p className="text-xs text-text-secondary">{strength.domain}</p>
                  </div>
                </div>
              </motion.div>
            ))}
                ...
                {domain.characteristics.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-2">
                    {domain.characteristics.map((char, idx) => (
                      <span key={idx} className="text-caption bg-bg-card text-text-secondary px-2 py-1 rounded-md border border-border-light">
                        {char}
                      </span>
                    ))}
                  </div>
          ...
          <div className="space-y-4">
            {reportData.strengthInterpretations.map((interpretation, index) => (
              <motion.div
                ...
                className="bg-bg-secondary rounded-xl border border-border-light overflow-hidden shadow-inner-soft"
              >
                <div className="p-5">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="font-bold text-text-primary text-body">{interpretation.name}</h3>
                      <p className="text-xs text-text-secondary">{interpretation.domain}</p>
                    </div>
                    <div className="w-10 h-10 rounded-full bg-brand/10 flex items-center justify-center shadow-soft">
        ...
        >
          ...
          <div className="bg-bg-secondary rounded-xl p-6 border border-border-light shadow-inner-soft space-y-4">
            ...
            {reportData.comboInterpretation.potentialTraps.length > 0 && (
              <div className="bg-status-warning/5 rounded-lg p-4 border border-status-warning/10 shadow-soft">
                <h3 className="font-medium text-status-warning text-body flex items-center gap-2">
            ...
            {reportData.comboInterpretation.synergies.length > 0 && (
              <div>
                <h3 className="font-medium text-accent text-body flex items-center gap-2">
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
        ...
        >
          ...
          <div className="space-y-3">
            {reportData.keyInsights.map((insight, index) => (
              <div key={index} className="bg-bg-secondary rounded-lg p-4 border border-border-light flex items-start gap-3 shadow-inner-soft">
                <div
                  className="w-2.5 h-2.5 rounded-full mt-2 flex-shrink-0 bg-brand shadow-soft"
                />
                <p className="text-text-primary text-body-sm">{insight}</p>
              </div>
            ))}
          </div>
        </motion.section>
        ...
        >
          ...
          <div className="space-y-3">
            {reportData.suggestedPaths.map((path) => (
              <div key={path.path} className="bg-accent/5 rounded-lg p-4 border border-accent/20 shadow-soft flex items-start gap-3">
                <span className="text-xl text-accent flex-shrink-0">💡</span>
                <div>
                  <h3 className="font-medium text-text-primary text-body mb-1">{path.title}</h3>
                  <p className="text-text-secondary text-body-sm">{path.reason}</p>
                </div>
              </div>
            ))}
          </div>
